...
SinglyLinkedListNode* mergeLists(SinglyLinkedListNode* head1, SinglyLinkedListNode* head2) {
    ...
    SinglyLinkedListNode* listHead =(SinglyLinkedListNode*)calloc(sizeof(SinglyLinkedListNode), 1);
    
    //pointer to the head
    SinglyLinkedListNode* list = listHead;

    //untill all values in head1 and head2 is passed
    while( !(!head1 && !head2) ) {

        //case currents nums from both heads are equals
        if((head1 && head2) && head1->data == head2->data) {
            //add te valfrom head1
            list->data = head1->data;
            
            //add one more node and go to him
            list->next = (SinglyLinkedListNode*)calloc(sizeof(SinglyLinkedListNode), 1);
            list = list->next;
            
            //add te valfrom head2
            list->data = head2->data;
            
            //next values in head1 and head2
            head1 = head1->next;
            head2 = head2->next;
            
        //case the current num from head2 is less from head1 or head1 not exist 
        } else if((head1 && head2 && (head1->data > head2->data)) || (head2 && !head1)) {
            list->data = head2->data;
            head2 = head2->next;
            
        //case the current num from head1 is less from head2 or head2 not exist
        }else   if((head1 && head2 && (head2->data > head1->data)) || (head1 && !head2)) {
            list->data = head1->data;
            head1 = head1->next;
        }
        
        //check if are more values to add
        if(head1 || head2) {
